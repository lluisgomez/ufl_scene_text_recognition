...
struct model* model_;
  ...
  Mat prediction_render = Mat::ones(32,32,CV_8UC1) * 255; 
  int fontFace = FONT_HERSHEY_SIMPLEX;
  double fontScale = 2;
  int thickness = 3;
  int baseline=0;
  string text = string("")+ascii[predict_label-1];
  Size textSize = getTextSize( text, fontFace,
                            fontScale, thickness, &baseline);

  while ((textSize.width > prediction_render.cols)||(textSize.height > prediction_render.rows))
  {
  ...
  putText(prediction_render, text, textOrg, fontFace, fontScale,
        Scalar::all(0), thickness, 8);

  hconcat(img,prediction_render,img);
  imwrite("tmp.jpg",img);
  //imshow("image",img);
  //waitKey(-1);
